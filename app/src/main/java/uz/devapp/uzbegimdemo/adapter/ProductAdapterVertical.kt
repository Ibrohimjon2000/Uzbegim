...
import uz.devapp.uzbegimdemo.databinding.ItemProductVerticalBinding
import uz.devapp.uzbegimdemo.utils.PrefUtils
import uz.devapp.uzbegimdemo.utils.loadImage

interface ProductAdapterVerticalCallback {
    fun onSelectCategory(item: ProductListModel)
    fun onSelectFavorite(item: ProductListModel, holder: ProductAdapterVertical.Vh)
    fun onSelectCart(item: ProductListModel, holder: ProductAdapterVertical.Vh)
}

class ProductAdapterVertical(
    val items: List<ProductListModel>,
    val callback: ProductAdapterVerticalCallback
) :
    RecyclerView.Adapter<ProductAdapterVertical.Vh>() {

    inner class Vh(val binding: ItemProductVerticalBinding) : RecyclerView.ViewHolder(binding.root)
